...
import com.goods.DBUtilDaoImpToGood;
import com.goods.DBUtilDaoToGood;
...
public class BackStageServlet extends HttpServlet {
	...
	public void del(HttpServletRequest request, HttpServletResponse response){
		...
		DBDao.delUser(id);
		try {
			response.sendRedirect("backstage_BackStageServlet_userlistByPage");		
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}	
	...
	public void update2(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		/*request.getRequestDispatcher("backstage_BackStageServlet_userlistByPage").forward(request, response);*/
	    response.sendRedirect("backstage_BackStageServlet_userlistByPage");	
	}
	...
	public void goodlistByPage(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
	
		    request.setCharacterEncoding("utf-8");
		    response.setContentType("text/html;charset=utf-8"); 
		    HttpSession hs=request.getSession(); 
		    DBUtilDaoToGood dbu=new DBUtilDaoImpToGood();
		    List list1=dbu.queryAll();
		    int len=list1.size();
		    Page pa1=new Page(len);
		    if( hs.getAttribute("pageNo")!=null){
	...
	public void delGood(HttpServletRequest request, HttpServletResponse response){
		...
		DBUtilDaoToGood DBDao=new DBUtilDaoImpToGood();
		DBDao.delgood(id);
		try {
			response.sendRedirect("backstage_BackStageServlet_goodlistByPage");	
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}	
	...
	public void updateGood(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		int i=Integer.parseInt(id);
		Good us=(Good)dbu.query(i);
		request.setAttribute("good", us);
	...
	/*把表中的数据存到数据库中*/
	public void update2Good(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		request.setCharacterEncoding("utf-8");
		response.setContentType("text/html;charset=utf-8"); 
		DBUtilDaoToGood dbu=new DBUtilDaoImpToGood();
		String id=request.getParameter("goodsId");
		...
		System.out.println("传过来的"+goodsName);
		String goodsRest=request.getParameter("goodsRest");
		System.out.println(goodsRest);
		int goodsRest2=Integer.parseInt(goodsRest);
		String goodsPrice=request.getParameter("goodsPrice");
		double goodsPrice2=Double.parseDouble(goodsRest);
		String goodsImage=request.getParameter("goodsImage");
		String descript=request.getParameter("descript");
		System.out.println("这是descript"+descript);
		...
		good.setGoodsRest(goodsRest2);
		good.setGoodsPrice(goodsPrice2);
		good.setGoodsImage(goodsImage);
		good.setDescript(descript);
		good.setSortId(sortId);
		System.out.println("BAC"+good.toString());
		dbu.updategood(good);
		/*request.getRequestDispatcher("backstage_BackStageServlet_userlistByPage").forward(request, response);*/
	    response.sendRedirect("backstage_BackStageServlet_goodlistByPage");	
	}
